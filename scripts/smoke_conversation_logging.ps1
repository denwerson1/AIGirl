...
function Get-NssmEnvExtra([string]$SvcName) {
  ...
  foreach ($line in $lines) {
    ...
    if ($line -match "^\s*([^=]+?)\s*=\s*(.*)\s*$") { $h[$matches[1].Trim()] = $matches[2] }
  }
  return $h
}
...
if ([string]::IsNullOrWhiteSpace($server) -or [string]::IsNullOrWhiteSpace($db) -or
  ...
  throw "Cannot read DB_* from NSSM AppEnvironmentExtra for service '$ServiceName'."
}
...
if (-not $resp.ok) { throw "POST /events returned ok=false" }
...
if ($cm.Length -gt 0) {
  ...
  Write-Host ""
  Write-Host "FAIL: No ConversationMessages row for this EventId."
